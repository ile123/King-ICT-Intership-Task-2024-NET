...
public class DummyJsonController : ControllerBase
{
    ...
    /// <summary>Gets all products from the DummyJson api and stores them in the db</summary>  
    /// <returns>A string that confirms that products where retrived from the DummyJson api and stored in the db</returns>  
    ...
    public async Task<ActionResult<ApiResponseDto<string>>> GetDataFromApi()
    {
        try
        {
            var externalApiData = await externalApiService.GetDataFromApi();
